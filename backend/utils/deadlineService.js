...
async function getDeadlineCounts(scope = {}) {
  ...
  if (scope.role === 'employee') {
    ...
    where.push(
      `(t.created_by = $${params.length + 1}
        OR a.manager_id = $${params.length + 2})`
    );
  ...
  const count = async (condition, values = []) => {
  ...
  return {
    dueToday: await count(
      `t.due_date = $${params.length + 1}`,
      [today]
    ),

    dueThisWeek: await count(
      `t.due_date >= $${params.length + 1}
       AND t.due_date <= $${params.length + 2}`,
      [today, weekEnd]
    ),
...
async function processDeadlineNotifications() {
  ...
  for (const task of result.rows) {
    if (String(task.due_date).slice(0, 10) === tomorrow) {
      try {
    ...
    } else if (String(task.due_date).slice(0, 10) === today) {
      try {
        await notifyEmployee(
          task.assigned_to,
          'task',
      ...
      if (
        ...
        try {
          await notifyEmployee(
            ...
            'task',
            'Task overdue',
            `"${task.title}" assigned to ${task.assigned_name} is overdue.`,
            'task',
            task.id
          );
        } catch (error) {
